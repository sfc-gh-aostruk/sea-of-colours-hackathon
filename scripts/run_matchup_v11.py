"""Competitive validation sweep for tabula_v11.

Runs the three matchup families used to sign off a harness version, per-seat
agent bound via ``run_agent_turn(agent_label=...)`` (which wins in
``resolve_binding``), so a single season can mix v11 / v8 / v7 / heuristic:

  * mode=heur   — p1 tabula_v11  vs  p2 heuristic            (performance floor)
  * mode=mirror — p1/p2/p3 all tabula_v11                    (self-collision spread)
  * mode=v8v7   — p1 tabula_v11  vs  p2 tabula_v8  vs  p3 tabula_v7  (benchmark)

Per season it prints the per-day turn line, any GREEN auto-harvest log lines
(the s56 collapse signature — should be none for v11 now), and the final
per-seat scores + winner. Usage::

    SOC_BACKEND=snowflake python scripts/run_matchup_v11.py \
        --modes heur mirror v8v7 --seeds 69 2 56
"""
...
_MODES: Dict[str, Dict[str, Optional[str]]] = {
    "heur": {"p1": "tabula_v11", "p2": None},
    "heur2": {"p1": "tabula_v11", "p2": None, "p3": None},
    "heur3": {"p1": "tabula_v11", "p2": None, "p3": None, "p4": None},
    "mirror": {"p1": "tabula_v11", "p2": "tabula_v11", "p3": "tabula_v11"},
    "v8v7": {"p1": "tabula_v11", "p2": "tabula_v8", "p3": "tabula_v7"},
    "v7": {"p1": "tabula_v11", "p2": "tabula_v7"},
    "v8": {"p1": "tabula_v11", "p2": "tabula_v8"},
}
...
def _run_one(mode: str, seed: int, *, days: Optional[int], tag: str) -> dict:
    ...
    season_name = f"V11_{mode.upper()}_{tag}_s{seed}"
    ...
    rank_of_v11 = next(
        (i + 1 for i, (s, _) in enumerate(order) if seat_map[s] == "tabula_v11"),
        None,
    )
    print(f"  --> scores: " + ", ".join(
        f"{s}({seat_map[s] or 'heur'})={scores[s]}" for s in seats),
        flush=True)
    print(f"  --> winner: {','.join(winners)} | v11 rank={rank_of_v11} | "
    ...
    return {"mode": mode, "seed": seed, "scores": scores, "winners": winners,
            "v11_rank": rank_of_v11, "green": len(green_lines),
...
def main(argv: Optional[List[str]] = None) -> int:
    p = argparse.ArgumentParser(prog="run_matchup_v11")
    p.add_argument("--modes", nargs="*", default=["heur", "mirror", "v8v7"],
    ...
    for r in results:
        if "error" in r:
            ...
            print(f"  {r['mode']:>6} s{r['seed']:<3}: {sc} | winner="
                  f"{','.join(r['winners'])} v11_rank={r['v11_rank']} "
