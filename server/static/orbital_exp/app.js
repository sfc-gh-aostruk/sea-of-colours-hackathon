...
(() => {
  ...
  const MAP_ZOOM_KEY = "sea_of_colours:mapZoomPx_orbital";
  const MAP_ZOOM_MIN = 10;
  const MAP_ZOOM_MAX = 28;
  let MAP_ZOOM_DEFAULT = 14;  // mutable — station.js resets to fit-to-width value
  ...
  function refreshVersusButtonLabel() {
    ...
    soloAgentVersusBtn.textContent =
      `[ >> ${runtimeShortLabel(p1)} vs ${runtimeShortLabel(p2)} · ${turns} nights ]`;
  }
  ...
  function startTransmittingAnim(btn, template) {
     ...
     *  tracks the server's current phase (e.g. "RESOLVING NIGHT" →
  ...
  function startTransmitProgress(opts) {
    ...
    const interpretLog = (text) => {
      ...
      if (m) {
        return {
          tag: `D${m[1]}`,
          msg: "night resolving · praxis fires",
          kind: "info",
          headline: "[ %s RESOLVING NIGHT %s ]",
        };
      }
    ...
    const timeline =
      mode === "orbit"
        ...
        : [
            { at:  300, sig: "syn:hydrate", tag: "soc",   msg: "hydrating session blob",        kind: "info" },
            { at:  800, sig: "syn:bots",    tag: "bots",  msg: "bots plotting night moves",     kind: "info", headline: "[ %s BOTS PLOTTING %s ]" },
            { at: 1300, sig: "syn:night",   tag: "praxis", msg: "praxis resolving night",       kind: "info", headline: "[ %s RESOLVING NIGHT %s ]" },
            { at: 1800, sig: "syn:dawn",    tag: "dawn",  msg: "dawn breaks · orbit opens",     kind: "info", headline: "[ %s OPENING DAY %s ]" },
  ...
  function formatCellTip(c, xi, yi, meta) {
    ...
    if (c.entity && c.entity.ch != null) {
      ...
      if (eCh === "X" || eCh === "x") {
        ...
        const life =
          nr != null
            ? ` · expires in ${nr} night${Number(nr) === 1 ? "" : "s"}`
            : "";
  ...
  function paintObsVisionMap(host, dims, obsCells, ...seatCellArrays) {
    ...
    for (let y = 0; y < height; y++) {
      ...
      for (let x = 0; x < width; x++) {
        ...
        const tt = occupantsTitle(c);
        const occAttr = tt ? ` data-occ="${escAttr(tt)}"` : "";
        const entHtml = entityOverlayHtml(c.entity);
        const trailHtml = renderTrailOverlayHtml(c);
        parts.push(
  ...
  function fmtInventory(inv) {
    ...
    /** @type {string[]} */
    const lines = [];
    const cap = Number(inv.hoard_capacity) || 50;
  ...
  function renderStorageGrid(host, parcels, cap, opts) {
    ...
    for (let i = 0; i < cap; i++) {
      ...
      const refinedFrom = Array.isArray(row.refined_from)
        ...
        : [];
      const refinedAttr = refinedFrom.length
        ? ` data-refined-from="${escAttr(refinedFrom.join(","))}"`
        : "";
      parts.push(
        `<div class="cc-vault-slot cc-vault-slot--filled"` +
          ...
          scoreTierAttr +
          refinedAttr +
          ` style="background:${esc(bg)};color:${esc(fg)}${ownerStyle}">` +
  ...
  function fillStorageTooltip(tip, slot) {
    ...
    if (isShipped) {
      ...
      tip.innerHTML =
        ...
        ownerRow +
        `<div class="cc-vault-tooltip-row cc-vault-tooltip-row--meta">harvest night ${esc(day)}</div>` +
        rows.join("");
      return;
    }
  ...
  function renderVault(inv, shippedOverride) {
    ...
    const cap = Number(inv && inv.hoard_capacity) || 25;
    ...
    if (ccTabVaultMeta) {
      ccTabVaultMeta.textContent = `${String(hoard.length)}/${String(cap)}`;
    }
    if (ccTabShippedMeta) {
  ...
  function fullVaultPickupWarning() {
    ...
    const cap = Number(inv.hoard_capacity) || 0;
  ...
  async function runAgentVersus() {
    ...
    const baseLabel =
      `[ >> ${runtimeShortLabel(matchup.p1)} vs ${runtimeShortLabel(matchup.p2)} · ${maxTurns} nights ]`;
    versusRunning = true;
    versusCancel = false;
    ...
    try {
      ...
      setAgentStatus({
        state: "error",
        phase: `versus stopped at night ${i}`,
        detail: e.message || String(e),
      });
      if (!errSoloEl.textContent) {
        errSoloEl.textContent =
          "! versus stopped at night " + i + ": " + (e.message || e);
        errSoloEl.hidden = false;
      }
    } finally {
      if (versusCancel) {
        setAgentStatus({
          state: "done",
          phase: `versus STOPPED at night ${i} / ${maxTurns}`,
        });
      } else if (
        agentStatusEl &&
        ...
        setAgentStatus({
          state: "done",
          phase: `versus complete · ${i} night${i === 1 ? "" : "s"}`,
        });
      }
      versusRunning = false;
      versusCancel = false;
  ...
  function renderOrbitQueue() {
    ...
    if (orbitQueueCount) orbitQueueCount.textContent = `${n}/3`;
    if (orbitTabMeta) orbitTabMeta.textContent = `${n}/3`;
  }
  ...
  async function submitSoloOrbit() {
    ...
    const resolvingList = beginResolvingFrame({
      ...
      recap: orbitQueue.slice(0, 3).map(_describeOrbitActionForRecap),
    });
    ...
    try {
      ...
      const body = await submit(MY_SEAT, orbitQueue.slice(0, 3));
      clearOrbitQueue();
      progress.pushLine(
        "ready",
  ...
  document.querySelectorAll(".solo-add-orbit").forEach((btn) => {
    btn.addEventListener("click", () => {
      ...
      if (orbitQueue.length >= 3) {
        if (orbitErr) {
          orbitErr.textContent = "! orbit queue capped at 3 actions";
          orbitErr.hidden = false;
        }
        return;
      }
  ...
  async function submitSoloNight() {
    ...
    if (stranded.length && !strandWarnAcked) {
      ...
      errSoloEl.textContent =
        `\u26A0 ${list} will be DESTROYED at dawn (no pickup queued) ` +
  ...
  function syncReplayRowOnly() {
    ...
    if (!replayTicks.length) {
      ...
      if (replayDayBadge) {
        if (day) {
          replayDayBadge.textContent =
            days.length > 1 ? `D${day} / ${days.length} nights` : `D${day}`;
    ...
    if (tag === "step" || tag === "drop" || tag === "pickup") {
      for (const row of cur) {
        ...
        if (tag === "pickup" && beforePos && !afterPos) {
          return { kind: "pickup", from: beforePos, to: null, glyph: "X", fg: seatColor, owner: row.owner, damaged: Boolean(before?.damaged), idx: unitOrdinal(row.id) };
        }
      }
      return null;
    }
    ...
    function tick(now) {
      ...
      if (!midTriggered && rawT >= 0.5) {
        ...
        if (isBounce) {
          ...
          // drop deposit — station anim fires from call-site pre-hook
        } else {
  ...
  function runSingleDeltaAnimation(host, delta, prevFrame, curFrame) {
    if (delta.kind === "step") {
      ...
      let terrainStandin = null;
      if (delta.prevObsCell) {
        const oc = delta.prevObsCell;
        terrainStandin = document.createElement("span");
        terrainStandin.className = "harvest-terrain-standin";
        terrainStandin.style.background = oc.bg || "";
        if (oc.fg) terrainStandin.style.color = oc.fg;
        terrainStandin.textContent = oc.ch || "";
        toCell.appendChild(terrainStandin);
      }
      ...
      if (window._osLaunchDelay) { const _h = host, _d = delta; window.setTimeout(() => runOrbitalArcAnimation(_h, _d), window._osLaunchDelay); }
      else runOrbitalArcAnimation(host, delta);
      ...
      if (window._osLaunchDelay) window.setTimeout(_probeGo, window._osLaunchDelay);
      else _probeGo();
    }
  }
  ...
  function collectNewlyRevealedCells(host, prevFrame, curFrame, dropX, dropY) {
    ...
    const cur = (curFrame && curFrame.cells_player) || null;
    const prev = (prevFrame && prevFrame.cells_player) || null;
    if (!Array.isArray(cur) || !cur.length) return out;
    const RADIUS = 2; // probe vision radius (RULEBOOK §3.11)
    const R2 = RADIUS * RADIUS;
    for (let y = 0; y < height; y++) {
      const dyq = (y - dropY) * (y - dropY);
      if (dyq > R2) continue;
      for (let x = 0; x < width; x++) {
        const dx = x - dropX;
        if (dx * dx + dyq > R2) continue;
        const idx = y * width + x;
        const c = cur[idx];
        if (!c) continue;
  ...
  // Canvas-based ASCII noise burst. One burst (~680ms) of density chars
  // (░▒▓) flickering every frame across the whole map. Called once per
  // jammed turn by playChaffFx. CRT glitch easter egg preserved.
  function _renderChaffStatic() {
    ...
    cvs.style.cssText =
      `position:absolute;left:${mapRect.left - hostRect.left}px;` +
      `top:${mapRect.top - hostRect.top}px;` +
      `pointer-events:none;mix-blend-mode:screen;`;
    collisionFxLayer.appendChild(cvs);
    ...
    function tick(now) {
      ...
      if (elapsed > DUR) { cvs.remove(); return; }
      const t     = elapsed / DUR;
      ...
      for (let r = 0; r < rows; r++) {
        for (let c = 0; c < cols; c++) {
          ...
          ctx.fillStyle = `rgba(255,255,255,${(alpha * brt).toFixed(3)})`;
    ...
    // Easter egg: CRT glitch when user is in CRT graphics mode.
  ...
  function _describeOrbitActionForRecap(a) {
    ...
    switch (kind) {
      ...
      case "refine":         return `refine ${a.tier || ""}`.trim();
      case "ship_catapult":  return "catapult shipment";
      default:               return kind || "—";
    }
  }
  ...
  async function _playNightCinematic(startTickIdx) {
    ...
    try {
      // v1.1 — hand the map over from the resolving overlay, then open
      // with the PRAXIS BEGINS card before the night steps play.
      stageCinematicTakeover();
      await showMapTitleCard("PRAXIS BEGINS", "", { holdMs: 1750 });
      // Only bail if the user kicked off their own playback while the
      // opening card played; ``mainMapSource`` is still "live" here
      // because we haven't painted the first replay frame yet.
      if (replayTicker) {
        interrupted = true;
        return;
      }
      for (let t = startTickIdx; t < replayTicks.length; t++) {
        const tick = replayTicks[t];
        // v1.2 — skip DUSK slots: in a LIVE surface-resolve cinematic the
        // night already opened (the orbit-commit sunset + "NIGHT n begins"
        // card handled that). DAWN slots DO play — they drive the sunrise
        // wash + destruction FX; the recap pops once at the reveal (the
        // slot branch suppresses its auto-pop while ``_liveFxPlaying``).
        ...
        // finishes before we step away (or reveal).
        const holdMs = _lastDawnSweepMs > 0 ? Math.max(620, _lastDawnSweepMs) : 620;
        if (t < replayTicks.length - 1) {
  ...
  function formatReplaySlotLabel(frame) {
    ...
    if (day == null) return "";
    if (hour != null) {
      return `Night ${day} · praxis hour ${String(hour).padStart(2, "0")}`;
    }
    // Boundary frames: opening, dawn, swap-collision summary.
    if (frame.tag === "open") return `Night ${day} · praxis opens`;
    if (frame.tag === "dawn") return `Night ${day} · dawn`;
    return `Night ${day}`;
  }
  ...
  function reconstructVaultAtTick(tickIdx, seat) {
    ...
    let hoardParcels = [];
    if (lastHoardSnapshot && lastHoardSnapshot[seat]) {
      const snap = lastHoardSnapshot[seat];
  ...
  function paintReplayFrameOntoMain(mode) {
    ...
    if (typeof osOnReplayTick === "function" && tick) {
      ...
      const _rfTag = tick.slot ? null : (nightReplayFrames[tick.lastFrameIdx]?.tag || null);
      osOnReplayTick({ slot: tick.slot || null, day: tick.day, tag: _rfTag });
    }
    ...
    if (tick && (tick.slot === "dawn" || tick.slot === "dusk")) {
      ...
      if (replayCaptionEl) {
        replayCaptionEl.textContent = isDawn
          ? `[H22] DAWN · night ${day} ends — day breaks`
          : `[H00] DUSK · night ${day} begins`;
      }
      if (replaySlotEl) {
        replaySlotEl.textContent = isDawn
          ? `Night ${day} · DAWN` : `Night ${day} · DUSK`;
      }
      syncReplayScrubUi();
  ...
  async function refreshNightReplay({ playFx = false } = {}) {
    ...
    try {
      ...
      // orbital_exp hook — expose replay data for station panels
      ...
      // orbital_exp hover cards — expose report render functions so station.js
      // can build rich hover cards without duplicating report logic.
      window._osOpenReport           = (kind, day) => openReport(kind, day);
      window._osRenderStationObs     = (day, phase, lbl) => renderStationObsPanel(day, phase, lbl);
      window._osRenderOrbitalObs     = (day) => renderOrbitalObservations(day);
  ...
  function buildReplayTicks(frames) {
    ...
    while (i < frames.length) {
      ...
      if (currentDay !== lastDayBeforePush) {
        ...
        if (currentDay > 0) {
          // DUSK opening the new night → bind to its first frame.
          pushSlot("dusk", currentDay, i);
        }
      }
  ...
  function renderEndgame(summary) {
    ...
    const tallyHtml = `
      ...
      <div class="eg-tally-grid">
        ${players.map((p) => egTallyCard(p)).join("")}
      </div>`;
    ...
    return `
      <div class="eg-tally-card">
        ...
        <div class="eg-tally-rows">
          ...
          <div class="eg-trow"><span>red harvested</span><span>${p.red_harvested}</span></div>
          <div class="eg-trow"><span>green harvested</span><span>${p.green_harvested}</span></div>
          <div class="eg-trow"><span>blue harvested</span><span>${p.blue_harvested}</span></div>
  ...
  function renderPostOrbitBriefing(day) {
    ...
    actionsHead.textContent =
      `// ${playerTag(viewerSeat)} actions \u00B7 night ${day}`;
    ...
    if (!slotAssignments.length && !anyBids) {
      ...
      quiet.textContent =
        "// no catapult bids tonight \u00B7 quiet orbit";
  ...
  function renderPreOrbitRecap(day) {
    ...
    if (!Array.isArray(nightLog) || (!nightLog.length && !hasPre)) {
      ...
      empty.className = "cc-report-empty dim";
      empty.textContent =
        "// pre-orbital recap \u2014 waiting for the night to resolve\u2026";
      reportBodyEl.appendChild(empty);
      return;
    }
    ...
    reportBodyEl.appendChild(
      renderStationObsPanel(day, "pre", "STATION OBSERVATIONS \u00B7 end of night"),
    );
   ...
   *  no coords). */
  function renderOrbitalObservations(day) {
  ...
  function renderSurfaceLog(day) {
    ...
    const head = document.createElement("div");
    head.className = "cc-report-section-head";
    head.textContent = `SURFACE \u00B7 witnessed events \u00B7 night ${day}`;
    panel.appendChild(head);
    ...
    if (!chronicle.length) {
      ...
      dim.textContent = "// no surface events witnessed this night";
      box.appendChild(dim);
  ...
  function renderStationObsPanel(day, phase, headLabel) {
    const panel = document.createElement("div");
  ...
  function renderReplayDayTicks() {
    ...
    for (let i = 0; i < n; i += 1) {
      ...
      pip.title = isDawn
        ? `Night ${day} — DAWN${hasContent ? " · pre-orbital recap (click to view)" : ""}`
        : `Night ${day} — DUSK${hasContent ? " · post-orbital briefing (click to view)" : ""}`;
      pip.setAttribute("aria-label", `Night ${day} ${isDawn ? "dawn" : "dusk"}`);
  ...
  async function refreshSoloPlayerMap() {
    ...
    try {
      ...
      if (ccTabIntelMeta) {
        const hoardN =
          j.inventory && Array.isArray(j.inventory.hoard)
            ...
            : 0;
        const cap = (j.inventory && j.inventory.hoard_capacity) || 50;
      ...
      // Cap the hoard meta on the INTEL tab to the v0.8.0 capacity
      // (25) so the displayed denominator stays accurate when the
      // engine reports it.
      if (ccTabIntelMeta && j.inventory) {
        const hoardN = Array.isArray(j.inventory.hoard)
          ? j.inventory.hoard.length : 0;
        const cap = j.inventory.hoard_capacity || 25;
        ccTabIntelMeta.textContent = `${String(hoardN)}/${String(cap)}`;
      }
  ...
  async function pullAllMaps({ playFx = false } = {}) {
    mainMapSource = "live";
  ...
  async function pollLiveSync() {
    ...
    if (liveSyncBusy || inFlightSubmit) return;
    ...
    try {
      ...
      if (sig && sig !== liveSyncSig) {
        const first = liveSyncSig === "";
        liveSyncSig = sig;
        // A real change landed (opponent locked, phase/day rolled). Pull the
        // full state so the seat sees it. Skip the very first sample and any
        // time the user is scrubbing replay frames.
        if (!first && mainMapSource === "live") {
          await pullAllMaps({ playFx: true });
        }
      }
    } catch (_e) {
  ...
  // orbital_exp: expose zoom controls so station.js can set fit-to-width as 100%
  ...
  // orbital_exp: always start at default; station.js recalculates to fit-to-width once map renders
  ...
  // v0.9.11 — load the two orbit-report settings (both default ON).
  try {
