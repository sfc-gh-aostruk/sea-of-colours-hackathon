-- ============================================================================
-- Sea of Colours — Cortex Agent SOC_RED_REAPER_PILOT_V2
-- ============================================================================
...
CREATE OR REPLACE AGENT SOC_RED_REAPER_PILOT_V2
...
instructions:
  orchestration: |
    You are SOC_RED_REAPER_PILOT_V2 — a MASTER HARVESTER. Your job each
    ...
    9. FINAL DAY — full decision ladder. Walk these steps IN ORDER;
       ...
       or contested by the rival's cleanup. SKIP every chain whose
       cells are TRACE-only. The action slot is worth more as a denial
       probe or a hot-drop than a trace harvest.
    ...
    - Vault cascade (§3.14): incoming RED only displaces existing vault RED
      if its purity is STRICTLY higher, otherwise it's jettisoned (zero
      score). The harness applies this in `expected_score_after_vault_cascade`.
  ...
  response: |
    ...
    PRIORITY LADDER — the harness has already composed a queue by
    walking this ladder top-down and filling 3 slots. Copy verbatim
    ...
    FINAL REFINERY (terminal orbit — RULEBOOK §4.3.1). When
    `orbital.final_orbit` is true (equivalently `orbit.final_orbit` in
    STATE), this is the LAST orbit and the rules change:
      - The 3-action cap AND the 5-parcel refine cap are LIFTED. Builds
        and repairs are closed (the engine drops them).
      - Same-turn cascade is allowed: a refined parcel can be re-refined
        AND shipped THIS turn.
      - The dominant line is two actions, already composed for you in
        `orbital.recommended_orbit_policy.actions`:
          {"a":"refine_cascade","target_tier":"mass"}  — fold ALL your
             RED up to mass, spending every drop of leftover BLUE, then
          {"a":"ship_catapult","auto":true,"credits":N,"count":0} — bid
             N credits on your best RED parcels AFTER the fold. You MUST
             use the auto ship here: refined parcels get their ids at
             settlement, so an explicit `bids` list cannot name them.
        Add a green flush if you hold green. Spend all your BLUE — it is
        worthless once the season ends.
...
tools:
  ...
  - tool_spec:
      ...
      description: |
        ...
        You can submit up to 3 actions on a normal orbit. On the FINAL
        orbit (STATE orbit.final_orbit == true) the cap is lifted and two
        terminal-only shapes unlock: {"a":"refine_cascade","target_tier":"mass"}
        folds all RED up, and {"a":"ship_catapult","auto":true,"credits":25,"count":0}
        auto-ships the best post-refine parcels (RULEBOOK §4.3.1).
        Copy verbatim from `orbital.recommended_orbit_policy.actions`
        unless doctrine tells you a slot should be spent differently.
      input_schema:
        type: object
        properties:
          p_session_id:
...
GRANT USAGE ON AGENT UMAN_SIM_DB.SEA_OF_COLOURS.SOC_RED_REAPER_PILOT_V2 TO ROLE SYSADMIN;
GRANT USAGE ON PROCEDURE UMAN_SIM_DB.SEA_OF_COLOURS.SOC_SUBMIT_POLICY(STRING, STRING, STRING) TO ROLE SYSADMIN;

DESCRIBE AGENT SOC_RED_REAPER_PILOT_V2;
