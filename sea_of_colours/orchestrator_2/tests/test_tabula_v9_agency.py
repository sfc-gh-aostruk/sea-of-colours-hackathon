"""v9 Phase 2 — the agency layer (option registry / menu / resolver) and the
...
def _stage_of(response_format) -> str:
    ...
    if name == "soc_strategist_decision":
        return "plan"
    # ``soc_tabula_plan`` is the shared v7 mover schema; v9's moves-only mover
    # (A4) uses its own trimmed ``soc_tabula_v9_moves`` schema.
    if name in ("soc_tabula_plan", "soc_tabula_v9_moves"):
        return "mover"
    return "think"
...
def test_harness_run_wires_menu_to_thinker_and_execute_to_mover(monkeypatch):
    ...
    from sea_of_colours.orchestrator_2.harnesses.tabula_v9 import harness as h
    ...
    # ...and the mover received the EXECUTE recipe (not just posture).
    assert "EXECUTE THIS PLAN" in _FakeInvoker.prompts["mover"]
    assert "BLIND_GRAB" in _FakeInvoker.prompts["mover"]
...
def test_harness_recovers_empty_plan_from_prose(monkeypatch):
    ...
    assert "EXECUTE THIS PLAN" in _FakeInvokerEmptyPlan.prompts["mover"]
...
def test_harness_builds_directive_from_prose_when_parse_yields_none(monkeypatch):
    ...
    # resolved, and the mover still got an EXECUTE recipe.
    assert out["extras"]["selected_option_ids"] == ["BLIND_GRAB", "UNBEATEN_FLANK"]
    assert "EXECUTE THIS PLAN" in _FakeInvokerRambleNoDirective.prompts["mover"]
