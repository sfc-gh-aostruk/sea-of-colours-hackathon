...
def _view(*, mine=False, enemy_at=None):
    ...
    if enemy_at is not None:
        ...
        # Part B: a rival contest fires the aggressive BLIND_GRAB path only when
        # the seam is LIVE-confirmed; a fogged seam is CONTEST_DENY. These agency
        # tests exercise the aggressive path, so mark the seam LIVE.
...
# ── v11 menu rebuild: rich per-option economics + affordability + variants ──
def _red_chain_view(cells_purities):
    """A view whose world.live carries the given RED cells (for yield math)."""
    return {
        "world": {"width": 20, "height": 20, "live": [
            {"tile": "RED", "x": x, "y": y, "purity": p}
            for (x, y, p) in cells_purities
        ]},
        "orbit": {"probe_stock": 1},
        "my_assets": [{"id": "harvester_p1_1", "kind": "harvester",
                       "state": "orbit"}],
        "red_tiles": [], "blue_tiles": [], "last_night": {},
    }


def test_menu_annotates_chain_with_walk_yield_and_risk():
    """The enriched menu renders each option's walk, engine-scored yield, and
    collision risk inline — the single-source-of-truth requirement."""
    av = _red_chain_view([(5, 5, 255), (5, 6, 100)])
    ch = [{"unit": "harvester_p1_1", "drop_at": [5, 5],
           "cells": [[5, 5], [5, 6]], "tiers": ["pure", "vein"],
           "purities": [255, 100], "length": 2}]
    reg = agency.build_registry(agent_view=av, chain_hints=ch)
    block = agency.format_menu_block(reg, agent_view=av, probe_stock=1)
    assert "JUICE CHAINS" in block
    assert "walk: (5,5) -> (5,6)" in block
    assert "yield: red ~+825" in block          # 735 (pure) + 90 (vein)
    assert "collision risk: LOW" in block
    assert "crush: none" in block


def test_menu_without_agent_view_stays_terse():
    """No agent_view -> no econ annotation (backward-compatible terse menu)."""
    av = _red_chain_view([(5, 5, 255)])
    ch = [{"unit": "harvester_p1_1", "drop_at": [5, 5], "cells": [[5, 5]],
           "tiers": ["pure"], "purities": [255], "length": 1}]
    reg = agency.build_registry(agent_view=av, chain_hints=ch)
    block = agency.format_menu_block(reg)
    assert "[CH1]" in block
    assert "walk:" not in block and "collision risk:" not in block


def test_long_chain_gets_short_head_variant():
    """A long chain is offered full AND as a SHORT 'rich head' variant so the
    thinker can trade tail length for exposure/hold use."""
    av = _red_chain_view([(5, 5, 255), (5, 6, 100), (5, 7, 80),
                          (5, 8, 40), (5, 9, 30)])
    ch = [{"unit": "harvester_p1_1", "drop_at": [5, 5],
           "cells": [[5, 5], [5, 6], [5, 7], [5, 8], [5, 9]],
           "tiers": ["pure", "vein", "vein", "trace", "trace"],
           "purities": [255, 100, 80, 40, 30], "length": 5}]
    reg = agency.build_registry(agent_view=av, chain_hints=ch)
    assert "CH1" in reg and "CH1S" in reg
    assert reg["CH1S"].payload["cells"] == [[5, 5], [5, 6], [5, 7]]  # drop + 2
    assert reg["CH1"].payload["cells"] == ch[0]["cells"]             # untouched


def test_short_variant_absent_for_a_short_chain():
    av = _red_chain_view([(5, 5, 255), (5, 6, 100)])
    ch = [{"unit": "harvester_p1_1", "drop_at": [5, 5],
           "cells": [[5, 5], [5, 6]], "tiers": ["pure", "vein"],
           "purities": [255, 100], "length": 2}]
    reg = agency.build_registry(agent_view=av, chain_hints=ch)
    assert "CH1" in reg and "CH1S" not in reg


def test_menu_tags_unaffordable_probe_options_against_stock():
    """With zero probe stock, probe-spending options are tagged UNAFFORDABLE
    instead of being silently listed as if they were free."""
    av = _view(mine=False, enemy_at=[10, 10])
    reg = _registry(av)
    block = agency.format_menu_block(reg, agent_view=av, probe_stock=0)
    assert "UNAFFORDABLE (you have 0)" in block
...
def _stage_of(response_format) -> str:
    ...
    # v11's journal-aware decision schema renames the v7 ``soc_strategist_decision``
    # to ``soc_v11_strategist_decision`` (adds intent/reflection); accept both.
    if name in ("soc_strategist_decision", "soc_v11_strategist_decision"):
        return "plan"
    # ``soc_tabula_plan`` is the shared v7 mover schema; v10's moves-only mover
    # (A4) uses its own trimmed ``soc_tabula_v11_moves`` schema.
    if name in ("soc_tabula_plan", "soc_tabula_v11_moves"):
        return "mover"
    return "think"
    ...
    return {
        "agent_view": {
            ...
            "redsign": [{"center": [10, 10], "cells": [[10, 10, 1.0]],
                         "hour": 3, "mine": False}],
            # Part B: LIVE-confirmed rival seam so the harness exercises the
            # aggressive BLIND_GRAB contest (a fogged seam would be CONTEST_DENY).
            "red_tiles": [{"x": 10, "y": 10, "purity": 255, "freshness": "fresh"}],
            "blue_tiles": [],
...
def test_harness_run_wires_menu_to_thinker_and_execute_to_mover(monkeypatch):
    ...
    from sea_of_colours.orchestrator_2.harnesses.tabula_v11 import harness as h
