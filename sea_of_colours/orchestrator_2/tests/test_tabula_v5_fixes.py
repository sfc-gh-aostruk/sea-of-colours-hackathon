"""Unit tests for the tabula_v5 audit-fix batch.
...
from sea_of_colours.orchestrator_2.harnesses.tabula_v5 import probe_hints as ph
from sea_of_colours.orchestrator_2.harnesses.tabula_v5 import move_sanitizer as ms
from sea_of_colours.orchestrator_2.harnesses.tabula_v5.prompt import (
...
def test_sanitizer_keeps_crush_to_preserve_chain():
    # Drop lands on our own probe (>=2 nights, no loot on the exact cell) BUT
    # a harvest chain follows. Rerouting would orphan the chain, so keep the
    # crush and the full chain (banking the seam beats sparing spent vision).
    ...
    out, log = ms.sanitize_moves(moves, view)
    drop = next(m for m in out if m["a"] == "drop")
    assert tuple(drop["at"]) == (5, 5)  # crush kept, NOT rerouted
    steps = [tuple(m["to"]) for m in out if m["a"] == "step"]
    assert steps == [(6, 5), (7, 5), (8, 5)]  # full chain survives
    assert any("harvest chain follows" in s for s in log)
...
def test_plan_red_value_counts_only_visible_red():
    from sea_of_colours.orchestrator_2.harnesses.tabula_v5 import harness as hn
