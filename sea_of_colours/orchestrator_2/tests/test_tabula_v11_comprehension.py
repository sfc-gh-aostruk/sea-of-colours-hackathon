"""Unit tests for the tabula_v11 comprehension uplift.
...
def test_mover_with_recipe_hides_menu_and_swaps_full_utilization():
    """The PACKAGER mover loses the OPTION MENU (re-targeting bait) + FOG+ECHO and
    the 'deploy every unit' doctrine when it holds the thinker's committed recipe;
    the recipe-less fallback mover keeps the menu as its single source of truth.
    The retired standalone hint blocks appear in NEITHER path (v11 menu rebuild)."""
    av = _fleet_view()
    reg = agency_mod.build_registry(
        agent_view=av, chain_hints=_CHAIN_HINTS, probe_hints=_PROBE_HINTS)
    menu = agency_mod.format_menu_block(reg, agent_view=av, probe_stock=1)
    ...
    common = dict(agent_view=av, chain_hints=_CHAIN_HINTS,
                  probe_hints=_PROBE_HINTS, option_menu_block=menu)
    ...
    # The retired hint blocks are gone from BOTH paths — the menu subsumes them.
    for dead in ("HEURISTIC SUGGESTIONS", "PROBE PLACEMENT HINTS",
                 "HOT DROP HINTS", "TACTICAL OPTIONS"):
        assert dead not in no_recipe, dead
        assert dead not in with_recipe, dead
    # The rendered OPTION MENU block + FOG+ECHO present WITHOUT a recipe, hidden
    # WITH one. (Match the block header, not the bare words — doctrine now points
    # the planner at "the OPTION MENU below", so the phrase itself always occurs.)
    for marker in ("OPTION MENU (SELECT by ID", "FOG + ECHO"):
...
def test_thinker_shows_option_menu_not_hint_blocks():
    """The THINKER now selects from the single-source-of-truth OPTION MENU; the
    standalone hint blocks are retired. Fleet doctrine still fires for the
    planner."""
    av = _fleet_view()
    reg = agency_mod.build_registry(
        agent_view=av, chain_hints=_CHAIN_HINTS, probe_hints=_PROBE_HINTS)
    menu = agency_mod.format_menu_block(reg, agent_view=av, probe_stock=1)
    text = prompt_mod.build_prompt(
        mode="thinker", option_menu_block=menu,
        **_kwargs(agent_view=av, chain_hints=_CHAIN_HINTS,
                  probe_hints=_PROBE_HINTS))
    assert "OPTION MENU (SELECT by ID" in text
    assert "HEURISTIC SUGGESTIONS" not in text
    assert "PROBE PLACEMENT HINTS" not in text
