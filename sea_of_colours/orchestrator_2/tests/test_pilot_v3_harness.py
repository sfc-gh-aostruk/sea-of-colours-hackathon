"""Tests for the PILOT_V3 harness internals.

Covers:
* compile_candidates returns the expected shape.
* compile_candidates copes with sparse / empty views.
* harness._build_pilot_v3_prompt splices candidates into the STATE block.
"""
...
from sea_of_colours.orchestrator_2.harnesses.pilot_v3 import candidates as cc
from sea_of_colours.orchestrator_2.harnesses.pilot_v3 import memory as mem
from sea_of_colours.orchestrator_2.harnesses.pilot_v3 import harness as h
...
def test_harness_prompt_splices_candidates_into_state_json():
    ...
    body = h._build_pilot_v3_prompt("SESS", view)

    # Default-play hint must be present.
    assert "recommended_policy" in body
...
def test_harness_prompt_removes_direct_submit_instruction():
    view = {"agent_view": _view(my_assets=[_harv()])}
    body = h._build_pilot_v3_prompt("SESS", view)
...
def test_materialize_rejects_wrong_seat_and_uses_recommended_policy():
    compiled = {
        "candidates": {
            "recommended_policy": {
            ...
            "probes": [
                {"id": "P0", "moves": [{"a": "probe", "at": [9, 9]}]},
            ],
        }
    }
    moves, info = h._materialize_planning_policy(
        decision={"seat": "p2", "phase": "planning", "choice": "P0"},
        expected_seat="p1",
        expected_phase="planning",
        compiled=compiled,
    )

    assert moves == [{"a": "probe", "at": [1, 1]}]
    assert info["fallback_used"] is True
    assert info["fallback_reason"] == "wrong_seat"


def test_materialize_accepts_prefix_candidate_id():
    compiled = {
        "candidates": {
            "recommended_policy": {"moves": []},
            "harvest": [
                {"id": "H0_harvester_p1", "moves": [{"a": "pickup", "unit": "harvester_p1"}]},
            ],
        }
    }
    moves, info = h._materialize_planning_policy(
        decision={"seat": "p1", "phase": "planning", "choice": "H0"},
        expected_seat="p1",
        expected_phase="planning",
        compiled=compiled,
    )

    assert moves == [{"a": "pickup", "unit": "harvester_p1"}]
    assert info["fallback_used"] is False
    assert info["materialized_candidate_id"] == "H0_harvester_p1"


def test_harness_run_submits_only_current_seat_after_json_decision(monkeypatch):
    class FakeInvoker:
        calls = 0
        ...
        def invoke(self, prompt, **kwargs):
            FakeInvoker.calls += 1
            if "STRATEGIC" in self.agent_name:
                return {"ok": True, "response": "DIAGNOSIS: test\nPLAN: probe_seed\nTOP: P0\nRATIONALE: test", "elapsed_ms": 1}
            return {
                "ok": True,
                "response": '{"seat":"p2","phase":"planning","choice":"P0","rationale":"malicious wrong seat"}',
                "elapsed_ms": 1,
                "tool_calls": [],
            }

    view = {"phase": "planning", "agent_view": _view(my_assets=[_harv()])}
    monkeypatch.setattr(h, "TACTICAL_LLM_ENABLED", True)
    with patch.object(h, "CortexAgentInvoker", FakeInvoker), \
    ...
    assert submit.call_args.args[2] == "p1"
    assert out["extras"]["decision_validation"]["fallback_reason"] == "wrong_seat"


def test_harness_deterministic_executor_uses_strategist_top_choice(monkeypatch):
    class FakeInvoker:
        def __init__(self, *, agent_name):
            self.agent_name = agent_name
    ...
    compiled = {
        "candidates": {
            ...
            "probes": [{"id": "P0", "moves": [{"a": "probe", "at": [9, 9]}]}],
        }
    }
    view = {"phase": "planning", "agent_view": _view(my_assets=[_harv()])}
    monkeypatch.setattr(h, "TACTICAL_LLM_ENABLED", False)
    monkeypatch.setattr(h, "compile_candidates", lambda agent_view: compiled)
    with patch.object(h, "CortexAgentInvoker", FakeInvoker), \
         patch("sea_of_colours.snowpark.engine.submit_policy") as submit:
        submit.return_value = {"ok": True}
        out = h.run(store=None, session_id="S", player="p1", view=view)
    ...
    submit.assert_called_once()
    assert submit.call_args.args[2] == "p1"
    assert submit.call_args.args[3] == [{"a": "probe", "at": [9, 9]}]
    assert out["extras"]["decision"]["choice"] == "P0"
...
from sea_of_colours.orchestrator_2.harnesses.pilot_v3 import threat_assess as ta
...
def test_pure_cell_chain_outranks_vein_chain():
    ...
    Regression for the bug where PILOT_V3 picked a vein chain over a
...
from sea_of_colours.orchestrator_2.harnesses.pilot_v3 import orbit as orb
...
def test_orbit_probes_prioritized_over_harvester_when_no_red():
    ...
    # Fleet=2 → normal case (thin-fleet override does NOT apply).
...
def test_orbit_thin_fleet_prioritizes_harvester_over_probes():
    """v3 aggression patch: fleet < 2 → build_harvester precedes build_probe.

    Regression coverage for the fleet_rebuild plan. When we only have
    one harvester left, rebuilding matters more than probe coverage
    because a dead fleet can't harvest anything.
    """
    view = _orbit_view(credits=2000, probe_stock=1, red_visible=[],
                       harvester_cap_used=1)
    out = orb.compile_orbit_candidates(view)
    kinds = [a["a"] for a in out["recommended_orbit_policy"]["actions"]]
    assert "build_harvester" in kinds, kinds
    if "build_probe" in kinds:
        assert kinds.index("build_harvester") < kinds.index("build_probe"), (
            f"fleet_rebuild should precede probe stock top-up when fleet<2, got {kinds}"
        )
...
# ─────────────────────────────────────────────────────────────────────────
# V3 regression tests — the four behaviours we introduced.
# ─────────────────────────────────────────────────────────────────────────

from sea_of_colours.orchestrator_2.harnesses.pilot_v3 import orbit as v3_orbit


def _minimal_view():
    """Skeleton agent_view that compile_candidates can chew on."""
    return {
        "phase": "planning",
        "meta": {"phase": "planning", "day": 3, "seat": "p1"},
        "hud": {"credits": 1000, "scores": {"p1": 500, "p2": 300}},
        "my_assets": [],
        "entities": {"mine": []},
        "world": {},
        "navigation": {},
...
def test_v3_orbit_trivial_flag_present():
    """v3 exposes `orbital.trivial` on all orbit compiles."""
    view = _minimal_view()
    view["meta"]["phase"] = "orbit"
    out = v3_orbit.compile_orbit_candidates(view)
    assert "trivial" in out, "v3 orbit block must expose the 'trivial' flag"
    assert isinstance(out["trivial"], bool), "trivial must be bool"


def test_v3_orbit_trivial_true_when_nothing_pending():
    """Empty view with no vault pressure / no repairs → trivial=True."""
    view = _minimal_view()
    view["meta"]["phase"] = "orbit"
    out = v3_orbit.compile_orbit_candidates(view)
    # No vault escalation, no repair pool, no weapons available on
    # empty hud — should short-circuit as trivial.
    assert out["trivial"] is True, (
        "empty view should compile as trivial; got "
        f"vault_escalate={out['vault_pressure'].get('escalate')}, "
        f"repair={out['play_enablers']['repair_candidates']}"
    )


def test_v3_candidates_live_visible_drop_cells_present():
    """Top-level candidates block exposes the drop-cell whitelist."""
    view = _minimal_view()
    out = cc.compile_candidates(view)
    cand = out["candidates"]
    assert "live_visible_drop_cells" in cand, (
        "v3 candidates block must expose live_visible_drop_cells"
    )
    assert isinstance(cand["live_visible_drop_cells"], list)


def test_v3_probe_budget_own_active_coverage_present():
    """threat.probe_budget exposes own_active_coverage list + count."""
    view = _minimal_view()
    out = cc.compile_candidates(view)
    pb = out["threat"]["probe_budget"]
    assert "own_active_coverage" in pb, (
        "v3 probe_budget must expose own_active_coverage"
    )
    assert "own_active_probe_count" in pb, (
        "v3 probe_budget must expose own_active_probe_count"
    )
    assert isinstance(pb["own_active_coverage"], list)
    assert pb["own_active_probe_count"] == len(pb["own_active_coverage"])


def test_v3_probe_diversity_min_sep_bumped_to_9():
    """Probe candidates enforce min_sep >= 9 (2× radius=4) — no partial disk overlap.

    Seed the view with one own probe at (10,10). Any candidate produced
    should be at Manhattan distance >= 9 from (10,10); anything closer
    would have been filtered by the -600 penalty + skip-if-not-sep-ok gate.
    """
    view = _minimal_view()
    view["hud"]["credits"] = 5000
    # Provide fog cells for the probe candidate compiler to want to
    # cover. Give it a wide patch of fog far enough from (10,10) to
    # place a legitimate second probe.
    grid = _grid(30, 30)
    view["world"] = {"grid": grid}
    # Own probe already active in orbit.
    view["my_assets"] = [{
        "id": "probe_p1_1",
        "kind": "probe",
        "state": "deployed",
    ...
    view["entities"] = {"mine": [{"id": "probe_p1_1", "type": "probe", "pos": [10, 10]}]}
    # Also give some stock so probes are affordable.
    view["orbit_state"] = {"probe_stock": 2}
    out = cc.compile_candidates(view)
    # own_active_coverage must include (10,10)
    pb = out["threat"]["probe_budget"]
    coverage_pts = {tuple(e["at"]) for e in pb["own_active_coverage"]}
    assert (10, 10) in coverage_pts, (
        "own_active_coverage should include the deployed probe at (10,10); "
        f"got {coverage_pts}"
    )
    # Probe candidates (if any surfaced) must respect min_sep=9.
    for p in out["candidates"]["probes"]:
        at = p.get("at") or p.get("pt") or p.get("target")
        if not (isinstance(at, (list, tuple)) and len(at) == 2):
            continue
        d = abs(int(at[0]) - 10) + abs(int(at[1]) - 10)
        assert d >= 9, (
            f"probe candidate at {at} violates min_sep=9 vs own probe at (10,10); "
            f"manhattan={d}"
        )


def test_v3_orbit_sequential_budget_tracks_credits():
    """Sequential-budget compose returns running_credits_after / dropped_slots."""
    view = _minimal_view()
    view["meta"]["phase"] = "orbit"
    out = v3_orbit.compile_orbit_candidates(view)
    rec = out["recommended_orbit_policy"]
    # v3 fields must exist even when empty.
    assert "dropped_slots" in rec, "recommended_policy must expose dropped_slots"
    assert isinstance(rec["dropped_slots"], list)
    # Running-credit field is optional but should appear on any real compose.
    # For empty view, running_credits_after may or may not be populated but
    # the KEY should still be present.
    assert "running_credits_after" in rec or not rec.get("actions"), (
        "if actions were composed, running_credits_after must be present"
    )
