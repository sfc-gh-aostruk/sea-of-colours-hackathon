...
from sea_of_colours.game.weapons import (
    ...
    CHAFF_DURATION_HOURS as _ENGINE_CHAFF_HOURS,
)
...
CHAFF_HOURS = int(_ENGINE_CHAFF_HOURS)
...
def chaff_specs(agent_view: Mapping[str, Any]) -> int:
    """Chaff jam ``duration`` in hours, read off the view (RULEBOOK §4.9.5).

    A flare cancels EVERY seat's action for ``duration`` hours from the
    launch slot — the launcher included, immune only on the launch hour
    itself. So one flare costs the launcher a full ``duration``-hour
    window (the launch slot + ``duration - 1`` self-jammed slots). Read
    off ``orbit.weapon_specs.chaff`` so an engine retune reaches the menu
    prose and the doctrine arithmetic without an edit here.
    """
    spec = (
        ((agent_view.get("orbit") or {}).get("weapon_specs") or {}).get("chaff")
        or {}
    )
    return int(spec.get("duration_hours", CHAFF_HOURS) or CHAFF_HOURS)
