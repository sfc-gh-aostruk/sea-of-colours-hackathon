...
from sea_of_colours.orchestrator_2.harnesses.tabula_v5.rules import (
    RULES_SUMMARY,
)
from sea_of_colours.orchestrator_2.harnesses.tabula_v5.strategies import (
    STRATEGIES_CORE,
    DOCTRINE_BLUE,
    DOCTRINE_REDSIGN,
...
from sea_of_colours.orchestrator_2.harnesses.tabula_v5.orbit_wishlist import (
    Wishlist,
)
...
def format_hot_drop_hints_block(hints: Sequence[Mapping[str, Any]]) -> str:
    ...
    lines.append(
        ...
        "centre would crush the probe you just paid for. After you land, "
        "the probe reveals ~49 fresh cells: WALK the densest RED seam for "
        "as many hours as you can safely finish — do NOT stop at 1-3 "
        "steps — then pickup. A hot drop that walks only 2 cells wastes "
        "both the probe and the harvester's night.)"
    )
    return "\n".join(lines) + "\n"
...
def build_prompt(
    *,
    ...
    """Assemble the full Tabula v5 prompt.
