...
contained package — every import resolves inside ``tabula_v5``) and
...
New primary agent SOC_RED_REAPER_TABULA_V5 and finisher
SOC_RED_REAPER_TABULA_V5_FINISHER. v1-v4 remain isolated at their
own bindings.
...
Flow per turn:
  ...
  5. Invoke Cortex agent SOC_RED_REAPER_TABULA_V5 (single call)
...
from sea_of_colours.orchestrator_2.harnesses.tabula_v5 import (
    heuristic_chains,
    memory,
    move_sanitizer,
    opponent_weapons,
...
AGENT_NAME = "SOC_RED_REAPER_TABULA_V5"
INNER_AGENT_LABEL = "TABULA_V5"
...
FINISHER_AGENT_NAME = "SOC_RED_REAPER_TABULA_V5_FINISHER"
...
def run(
    *, store: Any, session_id: str, player: str, view: Mapping[str, Any],
    ...
    if phase == "orbit":
        ...
        # greens). Env var ``TABULA_V5_ORBIT_MODE=empty`` reverts to the
