"""Candidate compiler for SOC_RED_REAPER_PILOT_V3.

The PILOT_V3 agent's spec instructs it to *pick or compose* from a menu of
...
from sea_of_colours.orchestrator_2.harnesses.pilot_v4 import memory as _memory
from sea_of_colours.orchestrator_2.harnesses.pilot_v4 import threat_assess as _ta
from sea_of_colours.orchestrator_2.harnesses.pilot_v4 import combat as _combat
from sea_of_colours.orchestrator_2.harnesses.pilot_v4 import orbit as _orbit
...
# behaviour is unaffected — orchestrator_2's PILOT_V3 is the only
# consumer.
def _bfs_to_red(
    *,
    start: Tuple[int, int],
...
def _known_enemy_harvesters(agent_view: Mapping[str, Any]) -> List[Dict[str, Any]]:
    """Surface every enemy harvester we currently know about.

    v1.9 fix — the compiler previously hard-coded ``enemy_harvesters=[]``
    into the EMP-candidate compile step (with a "not surfaced reliably"
    TODO). That silently blocked every doctrine that needed an EMP
    target: fixture places a hostile harvester adjacent to us, we hold
    weapon_stock.emp >= 1, and yet zero emp_launch candidates were
    produced. Baseline #8 (``contested_pure_with_own_emp``) failed on
    exactly this path.

    Sources, in priority order:

    * ``world.grid`` cells with an ``entity.kind == "harvester"``
      belonging to a non-self owner — LIVE sightings inside our LOS.
    * ``world.live[]`` rows (list-mode world view) with the same entity
      shape but carrying explicit ``x, y`` fields and full ids.
    * ``competitor_intel.persistent_echoes[*]`` where ``kind`` names a
      harvester — stale probe-intel sightings.

    Returns dicts with ``at``, ``kind``, ``unit_id`` (if known),
    ``last_seen_day``, and ``source`` — same shape the EMP compiler's
    ``_units_in_cells`` helper reads.
    """
    me = str((agent_view.get("meta") or {}).get("player") or "")
    out: Dict[Tuple[int, int], Dict[str, Any]] = {}
    width, height = _world_dims(agent_view)
    ...
    # 1. list-mode world.live[] — has full entity id.
    for row in (world.get("live") or []):
        if not isinstance(row, Mapping):
            continue
        ent = row.get("entity")
        if not isinstance(ent, Mapping):
            continue
        if str(ent.get("owner") or "") == me:
            continue
        if str(ent.get("kind") or "") != "harvester":
            continue
        try:
    ...
    # 2. grid-mode world.grid[y][x] — cells carry a slim entity dict
    # without an id. Fall through only for positions not already seen
    # via list-mode.
    grid = world.get("grid")
    if isinstance(grid, list):
        for y, row in enumerate(grid):
    ...
    # 3. persistent echoes — stale probe intel. Older than live but
    # still targetable if the enemy hasn't moved.
    intel = agent_view.get("competitor_intel") or {}
    for r in intel.get("persistent_echoes") or []:
        if not isinstance(r, dict):
            continue
        if "harvester" not in str(r.get("kind") or "").lower():
            continue
        at = r.get("at")
        if not (isinstance(at, (list, tuple)) and len(at) == 2):
            continue
...
def _probe_candidates(
    *,
    ...
    # v3 — spatial diversity: require distance > 2×radius so two probes'
    # disks don't overlap at all. With radius=4 that's min_sep=9. This
    # replaces the softer v2 min_sep = max(6, radius) which allowed
    # partial disk overlap.
    min_sep = max(9, radius * 2)

    # Existing own probes: avoid stacking near them. v3 — cross-check
    # two independent sources so a probe deployed via a code path that
    # doesn't populate ``entities.mine`` still gets caught:
    #   1. ``entities.mine`` (view.py:_mine_entities) — normal path
    #   2. ``my_assets`` filtered to type=probe, state ∈ {deployed, surface}
    # v2 relied on (1) only and we observed same-cell re-launches.
    own_probes: List[Tuple[int, int]] = []
    _seen_own: Set[Tuple[int, int]] = set()

    def _add_own(x: Any, y: Any) -> None:
        try:
            pt = (int(x), int(y))
        except (TypeError, ValueError):
            return
        if pt in _seen_own:
            return
        _seen_own.add(pt)
        own_probes.append(pt)

    for ent in (agent_view.get("entities") or {}).get("mine") or []:
        if (ent or {}).get("type") == "probe":
            pos = ent.get("pos") or ent.get("at")
            if isinstance(pos, (list, tuple)) and len(pos) == 2:
                _add_own(pos[0], pos[1])
    for asset in agent_view.get("my_assets") or []:
        if not isinstance(asset, Mapping):
            continue
        kind = asset.get("kind") or asset.get("type") or asset.get("asset_type")
        if kind != "probe":
            continue
        state = asset.get("state")
        if state == "destroyed":
            continue
        at = asset.get("at") or asset.get("pos") or (
            [asset.get("last_seen_x"), asset.get("last_seen_y")]
            if asset.get("last_seen_x") is not None else None
        )
    ...
    for c in candidates:
        ...
        if not sep_ok:
            # v3 — bump from -200 to -600 so a colliding candidate is
            # always beaten by any non-colliding candidate (max plain
            # fog_yield*10 is ~490, so -600 dominates).
            score -= 600
        scored.append({
            **c,
            "min_sep_ok": sep_ok,
...
def compile_candidates(agent_view: Mapping[str, Any]) -> Dict[str, Any]:
    ...
    # v3 — own_active_coverage: an at-a-glance list of my currently
    # active probes so the agent can spot "don't launch a new probe
    # near this cell" without reconstructing from my_assets. Includes
    # nights_remaining so the agent can weight fresh vs about-to-expire.
    own_active_coverage: List[Dict[str, Any]] = []
    for asset in agent_view.get("my_assets") or []:
        if not isinstance(asset, Mapping):
            continue
        kind = asset.get("kind") or asset.get("type") or asset.get("asset_type")
        if kind != "probe":
            continue
        if asset.get("state") == "destroyed":
            continue
        at = asset.get("at") or asset.get("pos")
        if not (isinstance(at, (list, tuple)) and len(at) == 2 and at[0] is not None):
            continue
        entry = {
            "at": [int(at[0]), int(at[1])],
        }
        nr = asset.get("nights_remaining")
        if nr is not None:
            try:
                entry["nights_remaining"] = int(nr)
            except (TypeError, ValueError):
                pass
        own_active_coverage.append(entry)
    probe_budget["own_active_coverage"] = own_active_coverage
    probe_budget["own_active_probe_count"] = len(own_active_coverage)
    ...
    emp_launches = _combat.compile_emp_launch_candidates(
        agent_view=agent_view,
        enemy_probes=enemy_probes,
        # v1.9 — was hard-coded [] here; now surfaced via
        # _known_enemy_harvesters which walks world.live / world.grid /
        # persistent_echoes. Without this fix the EMP compiler saw no
        # targets and produced zero candidates for every fixture with
        # a visible hostile harvester.
        enemy_harvesters=_known_enemy_harvesters(agent_view),
    ...
    return {
        "candidates": {
            ...
            # v3 — self-verification aid: full list of live-visible cells
            # a harvester may LEGALLY be dropped on this hour. Any custom
            # drop cell the agent composes MUST be in this set or the
            # engine will reject the move ("no live sensor beacon"). The
            # agent's doctrine gates all custom drops on this list.
            "live_visible_drop_cells": sorted(
                [list(pt) for pt in _drop_targetable_cells(agent_view)]
            ),
        },
        "combat": {
