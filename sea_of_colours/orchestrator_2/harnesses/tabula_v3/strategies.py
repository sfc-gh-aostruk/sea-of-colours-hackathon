...
PROBE STRATEGY (nomadic discovery — advance the frontier, don't camp):
  ...
  USE MULTIPLE PROBES per night if possible — more probes = more
  revealed fog = more chances of finding good red to harvest next
  night. If you have probe_stock >= 2 and there is still fog on the
  map, launch AT LEAST 2 probes into different quadrants. Probes are
  cheap (1 hour each) and the compounding effect across nights is
  large — a probe you don't launch tonight is red you can't harvest
  tomorrow.
