...
PLAYS: Tuple[WeaponPlay, ...] = (
    WeaponPlay(
        play_id="SMOKE_PICNIC",
        weapon="emp",
        when="redsign_theirs",
        hour="super_early",
        combines_with="blind_grab",
        why=(
            "two or more rival probes sit inside one radius-2 diamond, so one "
            "launch at hour 1 blinds every eye on the seam for eight hours and "
            "we walk in and comb it while they cannot see the ground"
        ),
    ),
    ...
    WeaponPlay(
        ...
        combines_with="blind_grab",
        why=(
            "a rival who has just found a pure needs their own probe to see "
            "it, so killing that one eye at hour 2 refuses them the landing "
            "tonight and leaves the pure there for us"
        ),
    ),
)
