...
from sea_of_colours.orchestrator_2.harnesses.tabula_v4.rules import (
    RULES_SUMMARY,
)
from sea_of_colours.orchestrator_2.harnesses.tabula_v4.strategies import (
    STRATEGIES_SUMMARY,
    DOCTRINE_BEWARE_EMP,
    DOCTRINE_BEWARE_CHAFF,
)
from sea_of_colours.orchestrator_2.harnesses.tabula_v4.orbit_wishlist import (
    Wishlist,
)
...
def format_setup_night_advisory(
    ...
    lines = [
        ...
        "",
        f"  You have probe_stock={probe_stock}. A probe reveals an 81-cell "
        f"disk for 3 nights.",
        "",
...
def format_drop_legal_block(agent_view: Mapping[str, Any]) -> str:
    ...
    Engine rule: a drop cell must be (a) inside the Chebyshev-4 disk of
    an active friendly probe, OR (b) on a friendly harvester's tile or
    one of its four Manhattan-1 neighbours (the "plus"). Echo-only
    cells are NOT legal.

    Reasoning about probe expiry + disk math is exactly the kind of
    thing haiku gets wrong under time pressure (see days 6/7 of the
    arena_solo_normal run — LLM picked (13,10) and (38,23) which were
    outside every active disk). Rendering the constraint as a small
    lookup table means the LLM only has to CHECK, not COMPUTE.
    """
    entities = (agent_view.get("entities") or {}).get("mine") or []
    lines: List[str] = []
    ...
    for e in entities:
        ...
        probe_lines.append(
            f"  {pid} @ ({cx},{cy}): drop-legal if x in [{cx-4},{cx+4}] "
            f"AND y in [{cy-4},{cy+4}]  (81-cell disk, Chebyshev-4)"
        )
    ...
    if not probe_lines and not harv_lines:
        return (
            "DROP-LEGAL ZONES: NONE right now. You have no active probe "
            "and no harvester on the surface — but you CAN hot-drop: "
            "launch a probe at hour K, then drop inside its disk at "
            "hour K+1. Sequence in moves[] matters (probe FIRST). "
            "Probes are always legal — they dispel fog.\n"
        )

    lines.append(
        "DROP-LEGAL ZONES (every drop.at MUST be inside AT LEAST ONE of "
        "these — otherwise the engine rejects the move):"
    )
    if probe_lines:
        lines.append("  active probe disks:")
        lines.extend(probe_lines)
    if harv_lines:
        lines.append("  surface harvester plus-cells:")
        lines.extend(harv_lines)
    lines.append(
        "  HOT DROP: a NEW probe you launch at hour K also becomes a "
        "drop-legal disk for hours K+1 onward. Sequence in moves[] "
        "matters — probe FIRST, then drop."
    )
    return "\n".join(lines) + "\n"
...
def format_hot_drop_hints_block(hints: Sequence[Mapping[str, Any]]) -> str:
    ...
    for i, h in enumerate(hints):
        ...
        lines.append(
            f"  Combo {chr(ord('A')+i)}: {signal_label}"
        )
        lines.append(
    ...
    lines.append(
        "  (Requires the probe move to appear BEFORE the drop in your "
        "moves list. The engine refreshes live-vision each hour so the "
        "drop lands legally on the just-revealed cell. Crawl 1-3 more "
        "steps to grab neighbors, then pickup.)"
    )
    return "\n".join(lines) + "\n"
...
def format_last_night_block(agent_view: Mapping[str, Any]) -> str:
    ...
    if combat:
        lines.append(f"  combat_events ({len(combat)}): opponent action affected you")
...
def build_prompt(
    *,
    ...
    # Conditional doctrine — append EMP / chaff mitigation snippets ONLY
    # when at least one opponent's estimate flags that weapon. Keeps the
    # base STRATEGIES text weapon-agnostic so we don't burn prompt tokens
    # on doctrine the agent can't act on.
    strategies_text = STRATEGIES_SUMMARY
