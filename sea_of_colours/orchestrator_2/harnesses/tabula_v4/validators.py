...
def validate_moves(
    ...
    for m in moves:
        ...
        if act == "drop":
            ...
            # launched at hour K makes its Chebyshev-4 disk live vision
            # for moves at hour K+1 onward. Extend the live-vision set
            # so a subsequent drop inside that disk validates. Previously
            # we froze live_cells at turn-start and rejected legal
            # hot-drop chains ("drop @ (15,10) not in live vision" even
            # when the probe at (15,10) was one hour earlier).
            if ok:
                at = m.get("at")
