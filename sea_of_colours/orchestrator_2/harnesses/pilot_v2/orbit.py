"""Orbital candidate compiler + composer for PILOT_V2.
...
def _compose_recommendation(
    *,
    ...
    Every open slot gets a baseline productive spend (ship/refine/green
    flush) if nothing higher-priority claims it. No slots left empty.
    """
    actions: List[Dict[str, Any]] = []
    reasons: List[str] = []
    used_kinds: set[str] = set()

    def _try_append(action: Optional[Mapping[str, Any]], reason: str,
                    kind: str) -> bool:
        if action is None:
            return False
    ...
    if vault.get("escalate") and ship.get("action") is not None:
        _try_append(ship["action"], f"[vault {vault['hoard_used']}/{vault['hoard_max']}] ship {ship.get('ship_count', 0)} vein+ parcels", "ship_catapult")
        if refine.get("options") and len(actions) < MAX_ORBIT_ACTIONS:
            top = refine["options"][0]
            _try_append(top["action"], f"[vault escalation] refine {top['count']} {top['source_tier']}→{top['upgraded_to']}", "refine")

    # Priority 2 — repair any damaged harvester (may need multiple slots).
    for rc in repair:
        _try_append(rc["action"], f"repair {rc['unit_id']} ({rc['cost']}c)", f"repair:{rc['unit_id']}")
    ...
    # Priority 4 — harvester expansion.
    if harvester.get("action") is not None:
        v = harvester["verdict"]
...
def _compose_final_refinery(agent_view: Mapping[str, Any]) -> Dict[str, Any]:
    """v1.7 — terminal settlement orbit play (RULEBOOK §4.3.1).

    On the final orbit the engine drops builds/repairs, lifts the refine
    caps, and lets refined parcels ship the SAME turn. The doctrine is
    simple and dominant: fold ALL red up to mass (spend leftover BLUE),
    then AUTO-ship the best parcels (refined ids don't exist until
    settlement, so an explicit bid list can't name them), then flush any
    green. Returns the same ``recommended_orbit_policy`` shape.
    """
    ob = _get_orbit_block(agent_view)
    parcels = list(ob.get("hoard_parcels") or [])
    credits = int(ob.get("credits", 0) or 0)
    blue = int(ob.get("blue_purity_total", 0) or 0)
    red = [p for p in parcels if str(p.get("colour", "")).upper() == "RED"]
    refinable = [p for p in red if int(p.get("purity", 0) or 0) <= MASS_MIN_PURITY - 1]
    green_count = int(ob.get("green_owned_count", 0) or 0)

    actions: List[Dict[str, Any]] = []
    reasons: List[str] = []
    if blue > 0 and refinable:
        actions.append({"a": "refine_cascade", "target_tier": "mass"})
        reasons.append(f"refinery run → mass ({len(refinable)} parcels, {blue} BLUE)")
    est_n = max(1, len(red))
    bid_each = int(min(SHIP_BID_PER_PARCEL, credits // est_n)) if credits > 0 else 0
    if red or (blue > 0 and refinable):
        actions.append({
            "a": "ship_catapult", "auto": True, "credits": bid_each, "count": 0,
        })
        reasons.append(f"auto-ship best RED @ {bid_each}c (post-refine)")
    if green_count > 0:
        gf = _trace_to_green(agent_view)
        if gf.get("action") is not None:
            actions.append(gf["action"])
            reasons.append(f"green flush — {green_count} parcel(s)")

    return {
        "actions": actions,
        "total_actions": len(actions),
        "slot_reasons": reasons,
        "rationale_hint": "; ".join(reasons) or "final orbit: nothing to settle",
        "final_refinery": True,
    }
...
def compile_orbit_candidates(agent_view: Mapping[str, Any]) -> Dict[str, Any]:
    ...
    # v1.7 — the terminal orbit runs the uncapped Final Refinery; the normal
    # priority-ladder composer is replaced by the dominant cascade+auto-ship
    # line (builds/repairs are dropped by the engine there anyway).
    if is_final:
        recommended = _compose_final_refinery(agent_view)
    else:
    ...
    return {
        ...
        "max_actions": int(ob.get("actions_max", MAX_ORBIT_ACTIONS)),
    }
