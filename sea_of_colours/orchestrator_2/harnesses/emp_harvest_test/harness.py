"""emp_harvest_test — night entry point (clean fork of v11; changes land in Section 2).
...
both on the inference API) is native and on by default; ``EMP_HARVEST_TEST_SINGLE=1``
...
from sea_of_colours.orchestrator_2.harnesses.emp_harvest_test import prompt as prompt_mod
...
# ── Agent identity ─────────────────────────────────────────────────────
INNER_AGENT_LABEL = "EMP_HARVEST_TEST"
THINKER_AGENT_LABEL = "EMP_HARVEST_TEST_THINKER"
...
def _split_on() -> bool:
    """v12 runs the contained split by default; ``EMP_HARVEST_TEST_SINGLE=1`` disables."""
    return os.environ.get("EMP_HARVEST_TEST_SINGLE", "0").strip().lower() not in (
        "1", "true", "yes",
    )
...
def _autofill_on() -> bool:
    ...
    AGENT spends the shortfall and stays the author. Set ``EMP_HARVEST_TEST_AUTOFILL=1``
    to restore the old guarantee and measure the two against each other.
    """
    return os.environ.get("EMP_HARVEST_TEST_AUTOFILL", "0").strip().lower() in (
        "1", "true", "yes", "on",
    )
