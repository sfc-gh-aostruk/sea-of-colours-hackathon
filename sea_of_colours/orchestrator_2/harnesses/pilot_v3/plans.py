...
def prompt_line_for_strategist() -> str:
    """Render the plan list for the STRATEGIC agent prompt.

    One label + description per line so the strategist can scan quickly
    without a nested structure. Kept < 800 chars total so it doesn't
    bloat the strategist's already-tight prompt budget.
    """
    lines = ["Available plan labels — pick EXACTLY ONE:"]
    for label, info in PLAN_TAXONOMY.items():
