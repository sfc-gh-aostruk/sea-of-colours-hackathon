...
from sea_of_colours.orchestrator_2.harnesses.tabula_v6.rules import (
    RULES_SUMMARY,
)
from sea_of_colours.orchestrator_2.harnesses.tabula_v6.strategies import (
    STRATEGIES_CORE,
    DOCTRINE_BLUE,
    DOCTRINE_REDSIGN,
...
from sea_of_colours.orchestrator_2.harnesses.tabula_v6.probe_hints import (
    _redsign_centers,
    _enemy_probe_cells,
)
from sea_of_colours.orchestrator_2.harnesses.tabula_v6.validators import (
    _synthetic_green_cells,
    _green_hazard_cells,
)
from sea_of_colours.orchestrator_2.harnesses.tabula_v6.orbit_wishlist import (
    Wishlist,
)
...
def format_reflect_block(
    ...
    if actual is not None:
        lines.append(
            f"  you predicted \"{predicted}\"; the engine actually banked "
            f"{int(actual)} pts."
        )
    else:
        lines.append(
            f"  you predicted \"{predicted}\"; actual bank not yet resolved."
        )
    if losses:
    ...
    lines.append(
        ...
        "above, and gap_reason must name the loss (or, if none, say why the "
        "plan hit/missed). Do NOT leave it null."
    )
    return "\n".join(lines) + "\n"
...
def build_prompt(
    *,
    ...
    """Assemble the full Tabula v6 prompt.
