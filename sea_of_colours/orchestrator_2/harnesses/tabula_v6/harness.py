...
contained package — every import resolves inside ``tabula_v6``) and
...
New primary agent SOC_RED_REAPER_TABULA_V6 and finisher
SOC_RED_REAPER_TABULA_V6_FINISHER. v1-v4 remain isolated at their
own bindings.
...
Flow per turn:
  ...
  5. Invoke Cortex agent SOC_RED_REAPER_TABULA_V6 (single call)
...
from sea_of_colours.orchestrator_2.harnesses.tabula_v6 import (
    heuristic_chains,
    memory,
    move_sanitizer,
    opponent_weapons,
...
AGENT_NAME = "SOC_RED_REAPER_TABULA_V6"
INNER_AGENT_LABEL = "TABULA_V6"
...
FINISHER_AGENT_NAME = "SOC_RED_REAPER_TABULA_V6_FINISHER"
...
def run(
    *, store: Any, session_id: str, player: str, view: Mapping[str, Any],
    ...
    if phase == "orbit":
        ...
        # greens). Env var ``TABULA_V6_ORBIT_MODE=empty`` reverts to the
