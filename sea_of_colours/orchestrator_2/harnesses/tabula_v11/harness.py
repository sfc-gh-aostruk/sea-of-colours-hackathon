"""tabula_v11 — night entry point (true fork of v10).
...
THINKER_AGENT_LABEL = "TABULA_V11_THINKER"
...
def _split_on() -> bool:
    """v11 runs the contained split by default; ``TABULA_V11_SINGLE=1`` disables."""
    return os.environ.get("TABULA_V11_SINGLE", "0").strip().lower() not in (
        "1", "true", "yes",
    )
...
def run(
    *, store: Any, session_id: str, player: str, view: Mapping[str, Any],
    ...
    if selected_options:
        ...
        # the advisory LLM-mover block). ``avoid`` cells extend the forbidden
        # union (drops/steps there are refused); ``chaff_react`` caps every chain
        # so it lifts inside the safe window. Both are restriction-only, so they
        # can never re-target or extend a play — the compiler pattern is intact.
        ...
        packed, packager_log = packager.pack_recipe(
            ...
            probe_hints=probe_hints,
            supersede_hints=supersede_hints,
            forbidden_cells=hazard_cells | avoid_cells,
