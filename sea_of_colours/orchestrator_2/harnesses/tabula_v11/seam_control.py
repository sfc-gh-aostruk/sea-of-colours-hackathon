...
The two cases (from the user's poker book, weapon-free baseline):
  ...
  CASE 1 — the redsign is MINE (I discovered it, engine-truth ``mine=True``):
    * ``SMASH_GRAB``     H1 belly-flop ON the pure (auto-harvest), pick up fast —
                         the SURE play; I know where the pure is, secure it.
    * ``FULL_SWEEP``     H1 ALTERNATIVE — ride the WHOLE visible seam in one
                         outing (pures + mass): far more value, but a one-harvester
                         GAMBLE (a mid-walk collision/jam zeroes unlifted cargo;
                         risk rises with more players + later season). The thinker
                         weighs value vs risk against SMASH_GRAB.
    ...
    *,
    ...
    if len(comb_full) >= 2 and len(comb_full) > len(comb1):
        note_sweep = (
            ...
            "harvester you can do BOTH — smash with H1, sweep/secure with H2."
        )
        if core_visioned:
        ...
        sweep = SeamPattern(
            ...
            beacon=beacon,
            mine=True,
            title="Grab the whole seam (H1) — the greedy gamble",
    ...
    comb3 = _comb_path(
        probe3[0], probe3[1], drop3, width, height, green, value_cells,
...
def _rival_blind_attack_patterns(
    ...
    *,
    ...
    the "ahead / want-certainty" alternative. Zero probes is handled upstream
    (no rival pattern is offered — you cannot land on a fogged seam).
    """
    green = _known_green_cells(agent_view)
    emp = _emp_near(beacon, threat.get("emp_cells") or [])
    spacing = _EMP_CLOUD_HOURS if emp else 0
    ...
    ring = enumerate_value_ring(agent_view, beacon, probe1)
    drop1 = _best_core_cell(ring, probe1, beacon)
    value_cells = (
    ...
    comb1 = _comb_path(
        cover_center[0], cover_center[1], drop1, width, height, green, value_cells,
        max_steps=steps,
    )
    note1 = (
        "BLIND attack on a FRESH, MASS-RICH rival seam — a redsign is usually "
        "one night old and unharvested, so the pure is very likely still there. "
        "The smear is jittered, so DROP on the best guess and BLIND-WALK the "
        "seam (up to the full hold): a hit banks the jackpot + denies them, a "
        "miss still banks the surrounding vein/mass. Then pick up fast."
    )
    if supersede1 is not None:
        note1 += (
            f"; your covering probe lands ON the finder ({finder[0]},{finder[1]}) "
            "— it BLINDS them AND makes the drop legal (one probe, free denial)"
        )
    if threat.get("chaff_seen"):
        note1 += " (chaff about — keep the walk to the floor + a flank backup)"
    blind = SeamPattern(
        pattern_id="BLIND_GRAB",
        kind="BLIND_GRAB",
        beacon=beacon,
        mine=False,
        title="Blind grab (attack the fresh rival seam)",
        when="fogged but fresh + rich; blind the finder and blind-walk the seam",
        rationale=(
            "A rival redsign is nearly always one night old and unharvested, and "
            "the seam is mass-rich. So attack: land a covering probe (ON the "
            "finder to blind them when you can), drop on the best smear guess and "
            "blind-walk across the seam. A hit takes their jackpot AND denies "
            "them; a miss still banks nearby vein/mass. Bank fast."
        ),
        waves=[SeamWave(
            1, _H_SMASH, drop1, comb1, probe_at=wave1_probe, supersede=supersede1,
            direction=approach, unit_ordinal=0, note=note1, pickup_after=True,
            blind_walk=True,
        )],
    )

    # ── UNBEATEN_FLANK (H2): a second harvester + its OWN probe combs a
    ...
    comb_f = _comb_path(
        probe_f[0], probe_f[1], drop_f, width, height, green, value_cells,
    ...
    flank = SeamPattern(
        ...
        beacon=beacon,
        mine=False,
        title="Unbeaten flank (second harvester, own probe)",
...
def _rival_patterns(
    ...
    *,
    ...
    comb_w = _comb_path(
        probe_w[0], probe_w[1], drop_w, width, height, green, value_cells,
        **({"max_steps": _CHAFF_CHAIN_STEPS} if weapons else {}),
    )
    walk = SeamPattern(
        pattern_id="WALK_IN",
