...
def actual_yield(
    ...
    Prefers the authoritative replay-frame hoard delta (what actually entered the
    hoard last night); falls back to the view's ``my_parcels_banked`` channel when
    frames are unavailable or the delta is empty.
    """
    if frames and player:
        banked = banked_from_frames(frames, player)
        if banked:
            return _score_parcels(banked)
    ln = agent_view.get("last_night") or {}
    return _score_parcels(
        [p for p in (ln.get("my_parcels_banked") or []) if isinstance(p, Mapping)]
    )
...
def build(
    ...
    *,
    ...
    return {
        ...
        "what_you_saw": _what_you_saw(agent_view, seen_log),
        "degraded": degraded,
    }
