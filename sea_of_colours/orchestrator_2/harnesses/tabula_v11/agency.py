...
from sea_of_colours.orchestrator_2.harnesses.tabula_v11 import option_economics
from sea_of_colours.orchestrator_2.harnesses.tabula_v11 import value_pyramid
...
def build_registry(
    *,
    ...
    for i, spec in enumerate(
        value_pyramid.force_surface_grabs(
...
_KIND_HEADERS = [
    ("grab", "PRIORITY RED GRABS (mass/pure RED you can SEE or reach — the highest-value take, no probe; grab it FIRST)"),
    ...
    ("blue_grab", "HIGH-YIELD BLUE GRABS (rich blue you can SEE and grab with no risk — use when you NEED blue, or you have a spare harvester that would otherwise be wasted on low-yield red)"),
...
def _fmt_yield(yb: Mapping[str, Any]) -> str:
    ...
    # A fully-blind maneuver (all cells fog) — do not invent a yield.
    if not tiers and blue == 0 and green_n == 0 and unknown:
        return (
            f"yield: unknown ({unknown} blind fog cell(s) — the probe reveals "
            "them; drop cell auto-harvests whatever is under it)"
        )
...
def _econ_detail_lines(opt: Option, econ: Mapping[str, Any]) -> List[str]:
    ...
    lines.append(_fmt_yield(econ.get("yield") or {}))
    lines.append(
...
def format_menu_block(
    ...
    *,
    ...
    """Render the ID'd option menu for the THINKER to select from — the SINGLE
    source of truth for tonight's plays.
    ...
    header = (
        ...
        "order, into \"plan\"; the geometry is pre-filled for you). This menu is "
        "the SINGLE source of truth: every option carries its walk, expected "
        "yield, crushes, and collision risk — you do not need to reconstruct any "
        "of it."
    )
    ...
    for kind, kind_header in _KIND_HEADERS:
        ...
        for opt in opts:
            ...
            if agent_view is not None:
                ...
                for dl in _econ_detail_lines(opt, econ):
                    lines.append(f"       {dl}")
    return "\n".join(lines) + "\n"
