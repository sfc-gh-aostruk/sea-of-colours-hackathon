"""v9 agency layer — the curated option MENU and the ID -> geometry RESOLVER.
...
from sea_of_colours.orchestrator_2.harnesses.tabula_v9.seam_control import (
    SeamPattern,
)
...
def _hotdrop_option(idx: int, h: Mapping[str, Any]) -> Option:
    ...
    line = f"HD{idx}: probe {probe} -> drop {drop}"
    if walk:
        line += f" -> walk {walk}"
    line += f"  (unit {unit})"
    exec_lines = [line]
