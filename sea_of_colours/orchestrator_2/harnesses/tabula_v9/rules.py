"""Engine MECHANICS the agent must obey (tabula_v9 fork).

Physics does not change between v7/v8 and v9 — the rules are the engine's,
not the agent's. v9 re-exports the frozen ``RULES_SUMMARY`` verbatim so the
...
# SCORING CORRECTION (v9 hermetic fix — hyg-scoring). The engine scores a
...
# for v9 ONLY, so v7/v8 baselines stay byte-frozen for the comparison.
...
assert "2295" not in RULES_SUMMARY, "v9 scoring correction failed to apply"

# FULL-UTILIZATION CORRECTION (v9 hermetic fix — ws-full-utilization / audit A6).
...
# probes/weapons trade against harvest hours. Correct it for v9 only.
...
assert "budget is generous" not in RULES_SUMMARY, (
    "v9 full-utilization correction failed to apply"
)
