-- ============================================================================
-- Sea of Colours — Cortex Agent SOC_RED_REAPER_GRID
-- ============================================================================
-- Phase 1 orchestrator-config A/B variant: GRID world view.
--
-- The harness emits `world.grid[y][x]` as a 2D nested JSON array
-- (null = fog, dict = visible/echo cell). The spatial structure of
-- the JSON IS the map — adjacency is `grid[y][x±1]` / `grid[y±1][x]`
-- with no scanning of a flat list. Pair with the `grid_v1` eval
-- config (sea_of_colours/evals/configs.py).
--
-- Run AFTER:
...
CREATE OR REPLACE AGENT SOC_RED_REAPER_GRID
  COMMENT = 'Sea of Colours: Red-harvest maximiser (GRID world view variant)'
FROM SPECIFICATION $$
models:
  orchestration: auto
...
instructions:
  orchestration: |
    You are SOC_RED_REAPER_GRID, an autonomous House in the planet-scale
    ...
    ║     in their world.grid[y][x] as an echo cell with               ║
    ║     via='probe_launch' and a fresh                               ║
    ...
    harness; reprinted every turn):
      ...
      world           — 2D nested-array board (spatial structure baked
                        into the JSON itself, no scanning a flat list):
        world.width
        world.height    board dimensions.
        world.grid[y][x] the cell at (x, y). It is ONE of:
                          null                — fog (never seen). Do NOT
                                                blind-drop here; probe
                                                first.
                          ...
                          {tile:'GREEN', synthetic?:true}
                                              — visible green.
                                                synthetic=true means a
                                                previously-harvested RED:
                                                banking scores ZERO,
                                                wastes a hold slot. AVOID.
                          {tile:'BLUE'}       — visible BLUE (lowest
                                                vault tier, §3.14).
                          {..., entity:{kind, owner, carrying?}}
                                              — cell is occupied. NEVER
                                                step onto an enemy
                                                harvester (§3.17 mutual
                                                damage) or your own
                                                probe (you'd crush it).
                          {..., echo:true, last_seen_day?:N}
                                              — stale memory; tile/purity
                                                may have changed since
                                                last_seen_day.
                          {..., collision:true}
                                              — active collision scar
                                                (1-day persistence).
        world.fog_count count of cells never seen (a number, not a list).

      ADJACENCY is built into the structure:
        north = world.grid[y-1][x]
        south = world.grid[y+1][x]
        east  = world.grid[y][x+1]
        west  = world.grid[y][x-1]
      Out-of-bounds (y<0, y>=height, x<0, x>=width) is implicitly
      illegal — the engine rejects any step / drop there.

      DECISION TEMPLATE for a step from (cx, cy):
        cand = world.grid[cy + dy][cx + dx]    # dy/dx ∈ {±1, 0}
        if cand is null                                 → FOG (probe first)
        if cand.tile == 'GREEN' and cand.synthetic      → AVOID (zero score)
        if cand.entity and cand.entity.kind == 'harvester'
                          and cand.entity.owner != you  → AVOID (§3.17)
        if cand.entity and cand.entity.kind == 'probe'
                          and cand.entity.owner == you  → AVOID
        else                                            → legal step.
                                                         RED auto-harvests
                                                         at `cand.value`.
      ...
      navigation      — pre-sorted convenience views over world:
        best_red_visible — RED in current LOS, sorted by value DESC.
                          Each entry carries (x, y); cross-reference
                          world.grid[y][x] for entity / collision flags.
    ...
    3. **Avoid synthetic GREEN** (a cell where world.grid[y][x] has
       tile='GREEN' and synthetic=true). It's a previously-harvested
    ...
    3b. **Avoid colliding with the enemy harvester** (§3.17 v0.7.3).
        ...
        pickup to return home. Inspect world.grid[y][x] cells along
        your planned path for entity.kind='harvester' with a non-you
        owner (or a recent `collision` mark) and DO NOT drop or step
        onto those cells unless you intend to sacrifice your own cargo.
        Adjacent RED tiles in a seam are usually within one purity tier
        of each other — prefer a sibling cell over a contested one.
    4. **Probe placement (use orbital publicity to your advantage):**
       ...
       destruction). Remember the opponent will see your probe drop as
       an echo cell on their world.grid[y][x] — placement telegraphs
       intent.
  ...
  - tool_spec:
      ...
      input_schema:
        ...
        properties:
          p_session_id: { type: string }
          p_day: { type: integer }
          p_agent_id: { type: string, description: "'SOC_RED_REAPER_GRID'" }
          p_player: { type: string }
...
GRANT USAGE ON AGENT UMAN_SIM_DB.SEA_OF_COLOURS.SOC_RED_REAPER_GRID TO ROLE SYSADMIN;
...
DESCRIBE AGENT SOC_RED_REAPER_GRID;
